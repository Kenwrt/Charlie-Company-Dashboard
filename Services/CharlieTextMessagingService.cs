...
public interface ICharlieTextMessagingService
{
    ...
    Task SendOperationalAsync(string userId, string phoneNumber, string body, string idempotencyKey, CancellationToken cancellationToken);
}
...
public sealed class WrightCharlieTextMessagingService(IMessagingClient client) : ICharlieTextMessagingService
{
    ...
    public async Task SendOperationalAsync(string userId, string phoneNumber, string body, string idempotencyKey, CancellationToken cancellationToken) =>
        _ = await client.SendAsync(new SendTextRequest(phoneNumber, body, userId, idempotencyKey), cancellationToken);
}
...
public sealed class UnconfiguredCharlieTextMessagingService : ICharlieTextMessagingService
{
    ...
    public Task SendOperationalAsync(string userId, string phoneNumber, string body, string idempotencyKey, CancellationToken cancellationToken) =>
        throw new InvalidOperationException("Text messaging is not configured.");
}
